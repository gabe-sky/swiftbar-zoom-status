...
# <bitbar.title>Zoom Video State</bitbar.title>
# <bitbar.version>v1.0</bitbar.version>
# <bitbar.author>Gabe Schuyler</bitbar.author>
# <bitbar.author.github>gabe-sky</bitbar.author.github>
# <bitbar.desc>Shows your microphone's current mute state in Zoom.</bitbar.desc>
    ...
    tell application process "zoom.us"
      if exists (menu bar item "Meeting" of menu bar 1) then
        if exists (menu item "Stop Video" of menu 1 of menu bar item "Meeting" of menu bar 1) then
          set returnValue to ":video.fill: | sfcolor=#5d9657 | symbolize=True"
        else
          set returnValue to ":video.slash.fill: | sfcolor=#cf2323 | symbolize=True"
        end if
      else
        set returnValue to ":video: | sfcolor=#676f87 | symbolize=True"
      end if
    end tell
  end tell
else
  set returnValue to ":video: | color=#8c8c8c | symbolize=True"
end if
